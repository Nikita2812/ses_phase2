...
export async function runAudit(auditRequest) {
  const response = await fetch(`${API_BASE_URL}/api/v1/constructability/audit`, {
    method: 'POST',
...
export async function runQuickAudit(designData, requestedBy, projectId = null) {
  const response = await fetch(`${API_BASE_URL}/api/v1/constructability/audit/quick`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
...
export async function auditExecution(executionId, requestedBy, auditType = 'full') {
  const response = await fetch(
    `${API_BASE_URL}/api/v1/constructability/audit/execution/${executionId}?requested_by=${encodeURIComponent(requestedBy)}&audit_type=${auditType}`,
    { method: 'POST' }
  );
  if (!response.ok) {
    const error = await response.json();
...
export async function getAudit(auditId) {
  const response = await fetch(`${API_BASE_URL}/api/v1/constructability/audits/${auditId}`);
  if (!response.ok) {
    const error = await response.json();
...
export async function listAudits(projectId = null, executionId = null, limit = 20) {
  let url = `${API_BASE_URL}/api/v1/constructability/audits?limit=${limit}`;
...
export async function analyzeRebar(memberData) {
  const response = await fetch(`${API_BASE_URL}/api/v1/constructability/analyze/rebar`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function analyzeFormwork(memberData) {
  const response = await fetch(`${API_BASE_URL}/api/v1/constructability/analyze/formwork`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(memberData),
  });
...
export async function analyzeFull(designData, analysisDepth = 'standard') {
  const response = await fetch(
    `${API_BASE_URL}/api/v1/constructability/analyze/full?analysis_depth=${analysisDepth}`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function generateRedFlagReport(analysisResult, projectId = null, projectName = null) {
  let url = `${API_BASE_URL}/api/v1/constructability/report/red-flag`;
...
export async function generateMitigationPlan(analysisResult, projectId = null) {
  const response = await fetch(`${API_BASE_URL}/api/v1/constructability/report/mitigation-plan`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
...
export async function getAuditReport(auditId) {
  const response = await fetch(`${API_BASE_URL}/api/v1/constructability/report/${auditId}`);
  if (!response.ok) {
    const error = await response.json();
...
export async function listOpenFlags(projectId = null, severity = null) {
  let url = `${API_BASE_URL}/api/v1/constructability/flags`;
  const params = new URLSearchParams();
  if (projectId) params.append('project_id', projectId);
...
export async function resolveFlag(flagId, resolutionNotes, resolvedBy) {
  const response = await fetch(`${API_BASE_URL}/api/v1/constructability/flags/${flagId}/resolve`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
...
export async function acceptFlag(flagId, acceptanceNotes, acceptedBy) {
  const response = await fetch(`${API_BASE_URL}/api/v1/constructability/flags/${flagId}/accept`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
...
export async function getStatistics(projectId = null, days = 30) {
  let url = `${API_BASE_URL}/api/v1/constructability/stats?days=${days}`;
  if (projectId) url += `&project_id=${projectId}`;
...
export async function getHealth() {
  const response = await fetch(`${API_BASE_URL}/api/v1/constructability/health`);
  if (!response.ok) {
    const error = await response.json();
